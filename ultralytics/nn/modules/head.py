...
class Detect(nn.Module):
    ...
    def bias_init(self):
        ...
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.masks, 0)[:, 0]).long(), minlength=nc) + 1
...
class RTDETRDecoder(nn.Module):
    ...
    and class masks for objects in an image. It integrates features from multiple layers and runs through a series of
