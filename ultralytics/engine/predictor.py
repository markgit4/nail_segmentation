...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            if self.args.save or self.args.save_txt:
                (self.save_dir / "masks" if self.args.save_txt else self.save_dir).mkdir(parents=True, exist_ok=True)
        ...
        if self.args.save or self.args.save_txt or self.args.save_crop:
            nl = len(list(self.save_dir.glob("masks/*.txt")))  # number of masks
            s = f"\n{nl} label{'s' * (nl > 1)} saved to {self.save_dir / 'masks'}" if self.args.save_txt else ""
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: list[str]) -> str:
        ...
        self.txt_path = self.save_dir / "masks" / (p.stem + ("" if self.dataset.mode == "image" else f"_{frame}"))
