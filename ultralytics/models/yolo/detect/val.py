...
class DetectionValidator(BaseValidator):
    ...
    Attributes:
        ...
        lb (list[Any]): List for storing ground truth masks for hybrid saving.
        ...
        for si, pred in enumerate(preds):
            ...
            if self.args.save_txt:
                self.save_one_txt(
                    ...
                    self.args.save_conf,
                    pbatch["ori_shape"],
                    self.save_dir / "masks" / f"{Path(pbatch['im_file']).stem}.txt",
                )
    ...
    def print_results(self) -> None:
        ...
        if self.metrics.nt_per_class.sum() == 0:
            LOGGER.warning(f"no masks found in {self.args.task} set, can not compute metrics without masks")
