...
class FastSAMPredictor(SegmentationPredictor):
    ...
    Attributes:
        prompts (dict): Dictionary containing prompt information for segmentation (bboxes, points, masks, texts).
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        labels = self.prompts.pop("masks", None)
        ...
        for result in results:
            ...
            if points is not None:
                ...
                assert len(labels) == len(points), (
                    f"Expected `masks` with same size as `point`, but got {len(labels)} and {len(points)}"
                )
                point_idx = (
