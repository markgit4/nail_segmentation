...
class Predictor(BasePredictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        labels = self.prompts.pop("masks", labels)
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        ...
        Args:
            ...
            labels (np.ndarray | list | None): Point prompt masks with shape (N) or (N, num_points). 1 for foreground, 0 for background.
    ...
    def _inference_features(
        ...
        Args:
            ...
            labels (np.ndarray | list[int] | None): Point prompt masks with shape (N,). 1 = foreground, 0 = background.
    ...
    def _prepare_prompts(self, dst_shape, src_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        Raises:
            AssertionError: If the number of points don't match the number of masks, in case masks were passed.
        """
        ...
        if points is not None:
            ...
            # Assuming masks are all positive if users don't pass masks.
            if labels is None:
                labels = np.ones(points.shape[:-1])
            labels = torch.as_tensor(labels, dtype=torch.int32, device=self.device)
            assert points.shape[-2] == labels.shape[-1], (
                f"Number of points {points.shape[-2]} should match number of masks {labels.shape[-1]}."
            )
            points *= r
            if points.ndim == 2:
                # (N, 2) --> (N, 1, 2), (N, ) --> (N, 1)
    ...
    def inference_features(
        ...
        Args:
            ...
            labels (np.ndarray | list[int] | None): Point prompt masks with shape (N, ).
            masks (list[np.ndarray] | np.ndarray | None): Masks for the objects, where each mask is a 2D array.
            multimask_output (bool): Flag to return multiple masks for ambiguous prompts.
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def add_new_prompts(
        ...
        Args:
            ...
            labels (torch.Tensor, optional): The masks corresponding to the points.
    ...
    def _run_single_frame_inference(
        ...
        Args:
            ...
            point_inputs (dict | None): Input points and their masks.
...
class SAM2DynamicInteractivePredictor(SAM2Predictor):
    ...
    def update_memory(
        ...
        Args:
            ...
            labels (torch.Tensor | None): Tensor of shape (B, N) representing the masks for the input points.
    ...
    def track_step(
        ...
        Args:
            ...
            label (torch.Tensor | None): The masks corresponding to the points where 1 means positive clicks, 0 means negative clicks.
