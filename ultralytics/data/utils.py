...
def img2label_paths(img_paths: list[str]) -> list[str]:
    """Convert image paths to label paths by replacing 'images' with 'masks' and extension with '.txt'."""
...
def verify_image_label(args: tuple) -> list:
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                if keypoint:
                    assert lb.shape[1] == (5 + nkpt * ndim), f"masks require {(5 + nkpt * ndim)} columns each"
                    points = lb[:, 5:].reshape(-1, ndim)[:, :2]
                else:
                    assert lb.shape[1] == 5, f"masks require 5 columns, {lb.shape[1]} columns detected"
                    points = lb[:, 1:]
                # Coordinate points check with 1% tolerance
                assert points.max() <= 1.01, f"non-normalized or out of bounds coordinates {points[points > 1.01]}"
                assert lb.min() >= -0.01, f"negative class masks or coordinate {lb[lb < -0.01]}"

                # All masks
                max_cls = 0 if single_cls else lb[:, 0].max()  # max label count
                assert max_cls < num_cls, (
                    f"Label class {int(max_cls)} exceeds dataset class count {num_cls}. "
                    f"Possible class masks are 0-{num_cls - 1}"
                )
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    msg = f"{prefix}{im_file}: {nl - len(i)} duplicate masks removed"
            else:
                ne = 1  # label empty
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    """
    Visualize YOLO annotations (bounding boxes and class masks) on an image.

    This function reads an image and its corresponding annotation file in YOLO format, then
    draws bounding boxes around detected objects and masks them with their respective class names.
    ...
    Args:
        ...
        label_map (dict[int, str]): A dictionary that maps class IDs (integers) to class masks (strings).
...
class HUBDatasetStats:
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        def _round(labels):
            """Update masks to integer class and 4 decimal place floats."""
            if self.task == "detect":
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                self.stats[split] = {
                    ...
                    "masks": [{Path(k).name: v} for k, v in dataset.imgs],
                }
            else:
                ...
                self.stats[split] = {
                    ...
                    "masks": [{Path(k).name: _round(v)} for k, v in zip(dataset.im_files, dataset.labels)],
                }
