...
class BaseTransform:
    ...
    Methods:
        apply_image: Apply image transformations to masks.
        apply_instances: Apply transformations to object instances in masks.
    ...
    Examples:
        ...
        >>> masks = {"image": np.array(...), "instances": [...], "semantic": np.array(...)}
        >>> transformed_labels = transform(masks)
    """
    ...
    def apply_image(self, labels):
        """
        Apply image transformations to masks.

        This method is intended to be overridden by subclasses to implement specific image transformation
        logic. In its base form, it returns the input masks unchanged.

        Args:
            labels (Any): The input masks to be transformed. The exact type and structure of masks may
                vary depending on the specific implementation.

        Returns:
            (Any): The transformed masks. In the base implementation, this is identical to the input.
    ...
    def apply_instances(self, labels):
        """
        Apply transformations to object instances in masks.

        This method is responsible for applying various transformations to object instances within the given
        masks. It is designed to be overridden by subclasses to implement specific instance transformation
        logic.
        ...
        Returns:
            (dict): The modified masks dictionary with transformed object instances.

        Examples:
            >>> transform = BaseTransform()
            >>> masks = {"instances": Instances(xyxy=torch.rand(5, 4), cls=torch.randint(0, 80, (5,)))}
            >>> transformed_labels = transform.apply_instances(masks)
        """
        pass
    ...
    def apply_semantic(self, labels):
        ...
        Args:
            labels (Any): The input masks or semantic segmentation mask to be transformed.

        Returns:
            (Any): The transformed semantic segmentation mask or masks.
    ...
    def __call__(self, labels):
        ...
        to the input masks. It sequentially calls the apply_image and apply_instances methods to process the
        ...
        Returns:
            (dict): The input masks dictionary with transformed image and instances.

        Examples:
            >>> transform = BaseTransform()
            >>> masks = {"img": np.random.rand(640, 640, 3), "instances": []}
            >>> transformed_labels = transform(masks)
        """
...
class BaseMixTransform:
    ...
    probability-based application of transforms and manages the mixing of multiple images and masks.

    Attributes:
        dataset (Any): The dataset object containing images and masks.
        pre_transform (Callable | None): Optional transform to apply before mixing.
        p (float): Probability of applying the mix transformation.

    Methods:
        __call__: Apply the mix transformation to the input masks.
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        Args:
            dataset (Any): The dataset object containing images and masks for mixing.
            pre_transform (Callable | None): Optional transform to apply before mixing.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply pre-processing transforms and cutmix/mixup/mosaic transforms to masks data.
        ...
        Returns:
            (dict[str, Any]): The transformed masks dictionary, which may include mixed data from other images.
    ...
    def _mix_transform(self, labels: dict[str, Any]):
        ...
        Returns:
            (dict[str, Any]): The modified masks dictionary with augmented data after applying the mix transform.

        Examples:
            >>> transform = BaseMixTransform(dataset)
            >>> masks = {"image": img, "bboxes": boxes, "mix_labels": [{"image": img2, "bboxes": boxes2}]}
            >>> augmented_labels = transform._mix_transform(masks)
        """
    ...
    def _update_label_text(labels: dict[str, Any]) -> dict[str, Any]:
        """
        Update label text and class IDs for mixed masks in image augmentation.

        This method processes the 'texts' and 'cls' fields of the input masks dictionary and any mixed masks,
        creating a unified set of text masks and updating class IDs accordingly.
        ...
        Returns:
            (dict[str, Any]): The updated masks dictionary with unified text masks and updated class IDs.
        ...
        Examples:
            ...
            ... }
            >>> updated_labels = self._update_label_text(masks)
...
class Mosaic(BaseMixTransform):
    ...
    Methods:
        ...
        _mix_transform: Apply mixup transformation to the input image and masks.
        _mosaic3: Create a 1x3 image mosaic.
        _mosaic4: Create a 2x2 image mosaic.
        _mosaic9: Create a 3x3 image mosaic.
        _update_labels: Update masks with padding.
        _cat_labels: Concatenate masks and clips mosaic border instances.
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply mosaic augmentation to the input image and masks.
        ...
        Examples:
            >>> mosaic = Mosaic(dataset, imgsz=640, p=1.0, n=4)
            >>> augmented_data = mosaic._mix_transform(masks)
        """
    ...
    def _mosaic3(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            (dict[str, Any]): A dictionary with the mosaic image and updated masks. Keys include:
                - 'img' (np.ndarray): The mosaic image array with shape (H, W, C).
                - Other keys from the input masks, updated to reflect the new image dimensions.
    ...
    def _mosaic4(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        updates the corresponding masks for each image in the mosaic.

        Args:
            labels (dict[str, Any]): A dictionary containing image data and masks for the base image (index 0) and three
                additional images (indices 1-3) in the 'mix_labels' key.

        Returns:
            (dict[str, Any]): A dictionary containing the mosaic image and updated masks. The 'img' key contains the mosaic
                image as a numpy array, and other keys contain the combined and adjusted masks for all four images.
    ...
    def _mosaic9(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing the input image and its associated masks. It should have
                the following keys:
                ...
                - 'mix_labels' (list[dict]): A list of dictionaries containing information for the additional
                  eight images, each with the same structure as the input masks.

        Returns:
            (dict[str, Any]): A dictionary containing the mosaic image and updated masks. It includes the following keys:
                - 'img' (np.ndarray): The final mosaic image.
                - Other keys from the input masks, updated to reflect the new mosaic arrangement.
    ...
    def _update_labels(labels, padw: int, padh: int) -> dict[str, Any]:
        ...
        This method adjusts the bounding box coordinates of object instances in the masks by adding padding
        ...
        Returns:
            (dict): Updated masks dictionary with adjusted instance coordinates.

        Examples:
            >>> masks = {"img": np.zeros((100, 100, 3)), "instances": Instances(...)}
            >>> padw, padh = 50, 50
            >>> updated_labels = Mosaic._update_labels(masks, padw, padh)
        """
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        """
        Concatenate and process masks for mosaic augmentation.

        This method combines masks from multiple images used in mosaic augmentation, clips instances to the
        ...
        Returns:
            (dict[str, Any]): A dictionary containing concatenated and processed masks for the mosaic image, including:
                - im_file (str): File path of the first image in the mosaic.
                - ori_shape (tuple[int, int]): Original shape of the first image.
                - resized_shape (tuple[int, int]): Shape of the mosaic image (imgsz * 2, imgsz * 2).
                - cls (np.ndarray): Concatenated class masks.
                - instances (Instances): Concatenated instance annotations.
                - mosaic_border (tuple[int, int]): Mosaic border size.
                - texts (list[str], optional): Text masks if present in the original masks.
...
class MixUp(BaseMixTransform):
    ...
    Minimization](https://arxiv.org/abs/1710.09412). MixUp combines two images and their masks using a random weight.
    ...
    Methods:
        _mix_transform: Apply MixUp augmentation to the input masks.
    ...
    def __init__(self, dataset, pre_transform=None, p: float = 0.0) -> None:
        ...
        values and masks. This implementation is designed for use with the Ultralytics YOLO framework.
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply MixUp augmentation to the input masks.
        ...
        Examples:
            >>> mixer = MixUp(dataset)
            >>> mixed_labels = mixer._mix_transform(masks)
        """
...
class CutMix(BaseMixTransform):
    ...
    and adjusts the masks proportionally to the area of the mixed region.
    ...
    Methods:
        _mix_transform: Apply CutMix augmentation to the input masks.
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply CutMix augmentation to the input masks.

        Args:
            labels (dict[str, Any]): A dictionary containing the original image and label information.

        Returns:
            (dict[str, Any]): A dictionary containing the mixed image and adjusted masks.

        Examples:
            >>> cutter = CutMix(dataset)
            >>> mixed_labels = cutter._mix_transform(masks)
        """
...
class RandomPerspective:
    ...
    Examples:
        ...
        >>> masks = {"img": image, "cls": np.array([0, 1]), "instances": Instances(...)}
    ...
    def __init__(
        ...
        Examples:
            ...
            >>> result = transform(masks)  # Apply random perspective to masks
        """
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply random perspective and affine transformations to an image and its associated masks.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing image data and annotations.
                Must include:
                    'img' (np.ndarray): The input image.
                    'cls' (np.ndarray): Class masks.
                    'instances' (Instances): Object instances with bounding boxes, segments, and keypoints.
                May include:
                    'mosaic_border' (tuple[int, int]): Border size for mosaic augmentation.

        Returns:
            (dict[str, Any]): Transformed masks dictionary containing:
                - 'img' (np.ndarray): The transformed image.
                - 'cls' (np.ndarray): Updated class masks.
...
class RandomHSV:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Examples:
            ...
            >>> masks = {"img": np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)}
            >>> masks = hsv_augmenter(masks)
            >>> augmented_img = masks["img"]
        """
        img = labels["img"]
...
class RandomFlip:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Examples:
            >>> masks = {"img": np.random.rand(640, 640, 3), "instances": Instances(...)}
            >>> random_flip = RandomFlip(p=0.5, direction="horizontal")
            >>> flipped_labels = random_flip(masks)
        """
        img = labels["img"]
        instances = labels.pop("instances")
...
class LetterBox:
    ...
    corresponding masks and bounding boxes.
    ...
    Methods:
        __call__: Resize and pad image, update masks and bounding boxes.
    ...
    def __call__(self, labels: dict[str, Any] = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        ...
        aspect ratio and adding padding to fit the new shape. It also updates any associated masks accordingly.

        Args:
            labels (dict[str, Any] | None): A dictionary containing image data and associated masks, or empty dict if None.
            image (np.ndarray | None): The input image as a numpy array. If None, the image is taken from 'masks'.

        Returns:
            (dict[str, Any] | nd.ndarray): If 'masks' is provided, returns an updated dictionary with the resized and padded image,
                updated masks, and additional metadata. If 'masks' is empty, returns the resized
                and padded image.

        Examples:
            >>> letterbox = LetterBox(new_shape=(640, 640))
            >>> result = letterbox(masks={"img": np.zeros((480, 640, 3)), "instances": Instances(...)})
            >>> resized_img = result["img"]
            >>> updated_instances = result["instances"]
        """
        if labels is None:
    ...
    def _update_labels(labels: dict[str, Any], ratio: tuple[float, float], padw: float, padh: float) -> dict[str, Any]:
        """
        Update masks after applying letterboxing to an image.

        This method modifies the bounding box coordinates of instances in the masks
        to account for resizing and padding applied during letterboxing.

        Args:
            labels (dict[str, Any]): A dictionary containing image masks and instances.
            ratio (tuple[float, float]): Scaling ratios (width, height) applied to the image.
            padw (float): Padding width added to the image.
            padh (float): Padding height added to the image.

        Returns:
            (dict[str, Any]): Updated masks dictionary with modified instance coordinates.

        Examples:
            >>> letterbox = LetterBox(new_shape=(640, 640))
            >>> masks = {"instances": Instances(...)}
            >>> ratio = (0.5, 0.5)
            >>> padw, padh = 10, 20
            >>> updated_labels = letterbox._update_labels(masks, ratio, padw, padh)
        """
        labels["instances"].convert_bbox(format="xyxy")
...
class CopyPaste(BaseMixTransform):
    ...
    Methods:
        _mix_transform: Apply Copy-Paste augmentation to the input masks.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """Apply Copy-Paste augmentation to an image and its masks."""
...
class Albumentations:
    ...
    Methods:
        __call__: Apply the Albumentations transformations to the input masks.

    Examples:
        >>> transform = Albumentations(p=0.5)
        >>> augmented_labels = transform(masks)
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply Albumentations transformations to input masks.

        This method applies a series of image augmentations using the Albumentations library. It can perform both
        spatial and non-spatial transformations on the input image and its corresponding masks.
...
class Format:
    ...
    Methods:
        __call__: Format masks dictionary with image, classes, bounding boxes, and optionally masks and keypoints.
    ...
    Examples:
        ...
        >>> formatted_labels = formatter(masks)
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        DataLoader. It processes the input masks dictionary, converting annotations to the specified format and
        applying normalization if required.

        Args:
            labels (dict[str, Any]): A dictionary containing image and annotation data with the following keys:
                - 'img': The input image as a numpy array.
                - 'cls': Class masks for instances.
        ...
        Examples:
            >>> formatter = Format(bbox_format="xywh", normalize=True, return_mask=True)
            >>> masks = {"img": np.random.rand(640, 640, 3), "cls": np.array([0, 1]), "instances": Instances(...)}
            >>> formatted_labels = formatter(masks)
    ...
    def _format_segments(
        ...
        Args:
            ...
            cls (np.ndarray): Class masks for each instance.
        ...
        Returns:
            ...
            cls (np.ndarray): Updated class masks, sorted if mask_overlap is True.
...
class LoadVisualPrompt:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Process masks to create visual prompts.

        Args:
            labels (dict[str, Any]): Dictionary containing image data and annotations.

        Returns:
            (dict[str, Any]): Updated masks with visual prompts added.
        """
    ...
    def get_visuals(
        ...
        Args:
            category (int | np.ndarray | torch.Tensor): The category masks for the objects.
...
class RandomLoadText:
    ...
    Methods:
        __call__: Process the input masks and return updated classes and texts.

    Examples:
        >>> loader = RandomLoadText(prompt_format="Object: {}", neg_samples=(5, 10), max_samples=20)
        >>> masks = {"cls": [0, 1, 2], "texts": [["cat"], ["dog"], ["bird"]], "instances": [...]}
        >>> updated_labels = loader(masks)
        >>> print(updated_labels["texts"])
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        This method samples positive texts based on the existing class masks in the image, and randomly
        selects negative texts from the remaining classes. It then updates the class indices to match the
        new sampled text order.

        Args:
            labels (dict[str, Any]): A dictionary containing image masks and metadata. Must include 'texts' and 'cls' keys.

        Returns:
            (dict[str, Any]): Updated masks dictionary with new 'cls' and 'texts' entries.

        Examples:
            >>> loader = RandomLoadText(prompt_format="A photo of {}", neg_samples=(5, 10), max_samples=20)
            >>> masks = {"cls": np.array([[0], [1], [2]]), "texts": [["dog"], ["cat"], ["bird"]]}
            >>> updated_labels = loader(masks)
        """
        assert "texts" in labels, "No texts found in masks."
