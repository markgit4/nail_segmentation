...
def convert_coco(
    ...
    for p in save_dir / "masks", save_dir / "images":
    ...
    for json_file in sorted(Path(labels_dir).resolve().glob("*.json")):
        ...
        fn = Path(save_dir) / "masks" / lname  # folder name
...
def convert_segment_masks_to_yolo_seg(masks_dir: str, output_dir: str, classes: int):
    ...
    Notes:
        ...
        After execution, the masks will be organized in the following structure:
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    associated label from the original masks directory and writes new masks in YOLO OBB format to a new directory.
    ...
    Notes:
        ...
        After execution, the function will organize the masks into:
    ...
    for phase in {"train", "val"}:
        ...
        orig_label_dir = dota_root_path / "masks" / f"{phase}_original"
        save_dir = dota_root_path / "masks" / phase
...
def yolo_bbox2segment(im_dir: str | Path, save_dir: str | Path | None = None, sam_model: str = "sam_b.pt", device=None):
    ...
    Args:
        ...
        save_dir (str | Path, optional): Path to save the generated masks, masks will be saved
            into `masks-segment` in the same directory level of `im_dir` if save_dir is None.
    ...
    if len(dataset.labels[0]["segments"]) > 0:  # if it's segment data
        LOGGER.info("Segmentation masks detected, no need to generate new ones!")
        return

    LOGGER.info("Detection masks detected, generating segment masks by SAM model!")
    sam_model = SAM(sam_model)
    for label in TQDM(dataset.labels, total=len(dataset.labels), desc="Generating segment masks"):
        h, w = label["shape"]
        boxes = label["bboxes"]
        if len(boxes) == 0:  # skip empty masks
            continue
        boxes[:, [0, 2]] *= w
        boxes[:, [1, 3]] *= h
    ...
    save_dir = Path(save_dir) if save_dir else Path(im_dir).parent / "masks-segment"
    save_dir.mkdir(parents=True, exist_ok=True)
    for label in dataset.labels:
        ...
        with open(txt_file, "a", encoding="utf-8") as f:
            f.writelines(text + "\n" for text in texts)
    LOGGER.info(f"Generated segment masks saved in {save_dir}")
...
def create_synthetic_coco_dataset():
    ...
    This function downloads COCO masks, reads image filenames from label list files,
    ...
    shutil.rmtree(dir / "masks" / "test2017", ignore_errors=True)  # Remove test2017 directory as not needed
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    format with separate directories for images and masks. It supports parallel processing for efficient
    ...
    for split in sorted(splits):
        ...
        (dataset_dir / "masks" / split).mkdir(parents=True, exist_ok=True)
    ...
    async def process_record(session, semaphore, record):
        ...
        async with semaphore:
            ...
            label_path = dataset_dir / "masks" / split / f"{Path(original_name).stem}.txt"
