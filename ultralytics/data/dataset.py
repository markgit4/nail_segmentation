...
class YOLODataset(BaseDataset):
    """
    Dataset class for loading object detection and/or segmentation masks in YOLO format.
    ...
    Methods:
        cache_labels: Cache dataset masks, check images and read shapes.
        get_labels: Return dictionary of masks for YOLO training.
    ...
    def cache_labels(self, path: Path = Path("./masks.cache")) -> dict:
        """
        Cache dataset masks, check images and read shapes.

        Args:
            path (Path): Path where to save the cache file.

        Returns:
            (dict): Dictionary containing cached masks and related information.
        """
        x = {"masks": []}
        ...
        if msgs:
            LOGGER.info("\n".join(msgs))
        if nf == 0:
            LOGGER.warning(f"{self.prefix}No masks found in {path}. {HELP_URL}")
    ...
    def get_labels(self) -> list[dict]:
        """
        Return dictionary of masks for YOLO training.

        This method loads masks from disk or cache, verifies their integrity, and prepares them for training.
        ...
        if not labels:
            raise RuntimeError(
                f"No valid images found in {cache_path}. Images with incorrectly formatted masks are ignored. {HELP_URL}"
            )
...
class YOLOMultiModalDataset(YOLODataset):
    """
    Dataset class for loading object detection and/or segmentation masks in YOLO format with multi-modal support.
...
class GroundingDataset(YOLODataset):
    ...
    def verify_labels(self, labels: list[dict[str, Any]]) -> None:
        ...
        masks matches the expected count for known datasets. It performs validation
    ...
    def cache_labels(self, path: Path = Path("./masks.cache")) -> dict[str, Any]:
        """
        ...
        Returns:
            (dict[str, Any]): Dictionary containing cached masks and related information.
        """
        x = {"masks": []}
    ...
    def get_labels(self) -> list[dict]:
        """
        Load masks from cache or generate them from JSON file.
