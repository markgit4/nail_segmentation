...
class Analytics(BaseSolution):
    ...
    Attributes:
        ...
        color_mapping (dict[str, str]): Dictionary mapping class masks to colors for consistent visualization.
    ...
    def update_graph(
        ...
        if count_dict is None:
            ...
            if plot == "area":
                ...
                for label in labels:  # Map masks to colors
                ...
                # Create the legend using masks from the bars
                ...
                start_angle = 90
                # Create pie chart and create legend masks with percentages
                wedges, _ = self.ax.pie(
                    counts, labels=labels, startangle=start_angle, textprops={"color": self.fg_color}, autopct=None
                )
                legend_labels = [f"{label} ({percentage:.1f}%)" for label, percentage in zip(labels, percentages)]

                # Assign the legend using the wedges and manually created masks
