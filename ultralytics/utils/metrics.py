...
class ConfusionMatrix(DataExportMixin):
    ...
    Attributes:
        ...
        names (list[str]): The names of the classes, used as masks on the plot.
    ...
    def __init__(self, names: dict[int, str] = [], task: str = "detect", save_matches: bool = False):
        ...
        Args:
            names (dict[int, str], optional): Names of classes, used as masks on the plot.
    ...
    def process_cls_preds(self, preds: list[torch.Tensor], targets: list[torch.Tensor]) -> None:
        ...
        Args:
            preds (list[N, min(nc,5)]): Predicted class masks.
            targets (list[N, 1]): Ground truth class masks.
        """
    ...
    def process_batch(
        ...
        if gt_cls.shape[0] == 0:  # Check if masks is empty
            if not no_pred:
