...
def _scale_bounding_box_to_original_image_shape(
    ...
    This function rescales the bounding box masks to the original image shape.
...
def _format_ground_truth_annotations_for_detection(img_idx, image_path, batch, class_name_map=None) -> dict | None:
    ...
    bounding boxes, class masks, and other metadata for a specific image in the batch, and formats them for
    ...
    if len(bboxes) == 0:
        LOGGER.debug(f"Comet Image: {image_path} has no bounding boxes masks")
        return None
...
def _log_images(experiment, image_paths, curr_step: int | None, annotations=None) -> None:
    ...
    Args:
        ...
        annotations (list[list[dict]], optional): Nested list of annotation dictionaries for each image. Each
            annotation contains visualization data like bounding boxes, masks, and confidence scores.
    """
    if annotations:
